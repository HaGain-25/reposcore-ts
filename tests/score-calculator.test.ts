...
describe('ScoreCalculator 단위 테스트', () => {
   ...
   * 상한선(Capping) 제한 규칙이 정상적으로 동작하는지 검증합니다.
   */
  describe('최종 점수(totalScore) 및 제한 규칙 계산', () => {
    ...
    /**
     * 코드 기여(PR) 없이 무분별하게 이슈만 생성하여 점수를 얻는 것을 방지하기 위해,
     * PR이 없는 경우 이슈 기여 점수도 함께 0점 처리되는 상한선 규칙을 검증합니다.
     */
    test('PR이 아예 없고 Issue만 존재하는 사용자는 상한선 캡핑에 의해 0점 처리되어야 한다', () => {
      // 테스트 의도: 코드 기여(PR) 없이 다수의 이슈만 생성한 경우, 규칙에 따라 이슈 점수 반영이 제한되는지 검증합니다.
      const repoData: RepoData = {
        owner: 'test-owner',
        repo: 'test-repo',
        scoreData: [
